...
suite "IPv4 - ipv4 from string":
  test "parse valid IPv4 string":
    ...
    check ip[0] == 192 and ip[1] == 168 and ip[2] == 1 and ip[3] == 1

  test "parse IPv4 with zeros":
    let ip = ipv4("10.0.0.1")
    check ip[0] == 10 and ip[1] == 0 and ip[2] == 0 and ip[3] == 1

  test "parse maximum IPv4 values":
    let ip = ipv4("255.255.255.255")
    check ip[0] == 255 and ip[1] == 255 and ip[2] == 255 and ip[3] == 255

  test "parse minimum IPv4 values":
    let ip = ipv4("0.0.0.0")
    check ip[0] == 0 and ip[1] == 0 and ip[2] == 0 and ip[3] == 0

  test "parse localhost":
    let ip = ipv4("127.0.0.1")
    check ip[0] == 127 and ip[1] == 0 and ip[2] == 0 and ip[3] == 1
